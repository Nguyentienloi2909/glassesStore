...
public class UsersDAO {
	...
	public boolean addAccountAdmin(Users user) throws Exception {
		DBConnect cs = new DBConnect();
		cs.KetNoi();
		String sql = "INSERT INTO taikhoan(tentaikhoan,email,matkhau,phanquyen,tenhienthi,hinhanh) VALUES (?,?,?,1,?,?) ";
		try {
			PreparedStatement cmd = cs.cn.prepareStatement(sql);
			cmd.setString(1, user.getTenTaiKhoan());
			cmd.setString(2, user.getEmail());
	...
	public boolean editPhanQuyenAccount(int id) {
	    ...
	    try {
	        cs.KetNoi();
	        String sql = "UPDATE taikhoan SET phanquyen = 1 WHERE id = ?";
	...
	public boolean editDeletePhanQuyenAccount(int id) {
	    ...
	    try {
	        cs.KetNoi();
	        String sql = "UPDATE taikhoan SET phanquyen = 0 WHERE id = ?";
	...
	public static void main(String[] args) {
		...
		System.out.println(usD.editDeletePhanQuyenAccount(24));
	}
